...
Unit-level model tests use moto (via mock_dynamodb fixture).
HTTP view tests use Django's test client against the same mocked DynamoDB.
"""
...
class TestPostModel:
    ...
    def test_default_published_false(self):
        p = Post.objects.create(title="Draft", slug="draft", author_pk=self.author.pk)
        assert p.published is False

    def test_tags_list(self):
        p = Post.objects.create(title="Tagged", slug="tagged",
                                author_pk=self.author.pk, tags=["python", "aws"])
        fetched = Post.objects.get(pk=p.pk)
        assert "python" in fetched.tags

    def test_post_author_property(self):
        p = Post.objects.create(title="T", slug="t", author_pk=self.author.pk)
        assert p.author.pk == self.author.pk

    def test_filter_by_author_pk(self):
        other = Author.objects.create(username="other")
        Post.objects.create(title="Mine",  slug="mine",  author_pk=self.author.pk)
        Post.objects.create(title="Other", slug="other", author_pk=other.pk)
        results = list(Post.objects.filter(author_pk=self.author.pk))
        assert len(results) == 1
...
class TestCommentModel:
    ...
    def _setup(self, mock_dynamodb):
        ...
        self.post = Post.objects.create(title="T", slug="t", author_pk=author.pk)
    ...
    def test_comment_post_property(self):
        c = Comment.objects.create(post_pk=self.post.pk, author_name="X", body="Y")
        assert c.post.pk == self.post.pk

    def test_filter_by_post_pk(self):
        other_post = Post.objects.create(title="Other", slug="other", author_pk=self.post.author_pk)
        Comment.objects.create(post_pk=self.post.pk, author_name="A", body="On post")
        Comment.objects.create(post_pk=other_post.pk, author_name="B", body="On other")
        results = list(Comment.objects.filter(post_pk=self.post.pk))
        assert len(results) == 1
...
class TestPostViews:
    ...
    def test_create_post(self, client):
        ...
        resp = client.post(
            "/api/posts/",
            data=json.dumps({"title": "T", "slug": "t", "author_pk": a.pk}),
            content_type="application/json",
        )
        assert resp.status_code == 201
    ...
    def test_list_posts(self, client):
        a = self._author()
        Post.objects.create(title="P1", slug="p1", author_pk=a.pk)
        Post.objects.create(title="P2", slug="p2", author_pk=a.pk)
    ...
    def test_list_filter_by_author(self, client):
        a = self._author()
        other = Author.objects.create(username="other")
        Post.objects.create(title="Mine",  slug="mine",  author_pk=a.pk)
        Post.objects.create(title="Other", slug="other", author_pk=other.pk)
        resp = client.get(f"/api/posts/?author_pk={a.pk}")
    ...
    def test_retrieve_increments_views(self, client):
        a = self._author()
        p = Post.objects.create(title="T", slug="t", author_pk=a.pk)
    ...
    def test_retrieve_includes_comments(self, client):
        a = self._author()
        p = Post.objects.create(title="T", slug="t", author_pk=a.pk)
        Comment.objects.create(post_pk=p.pk, author_name="R", body="Nice!")
        resp = client.get(f"/api/posts/{p.pk}/")
        assert len(resp.json()["comments"]) == 1

    def test_update_post(self, client):
        a = self._author()
        p = Post.objects.create(title="Old", slug="old", author_pk=a.pk)
        resp = client.put(
            f"/api/posts/{p.pk}/",
    ...
    def test_delete_post_cascades_comments(self, client):
        a = self._author()
        p = Post.objects.create(title="T", slug="t", author_pk=a.pk)
        c = Comment.objects.create(post_pk=p.pk, author_name="R", body="!")
...
class TestCommentViews:
    ...
    def test_delete_comment(self, client):
        _, p = self._setup()
        c = Comment.objects.create(post_pk=p.pk, author_name="R", body="B")
